...
// movimiento y escala sobre elementos interactivos. Portado del portfolio (React +
// next-intl) a vanilla TS sin el pill de labels. Adapta su color al tema de la
// sección bajo el puntero ([data-aa-section-theme]).
...
export function initCursor(root: HTMLElement): (() => void) | void {
  ...
  // Al hacer scroll (Lenis o nativo) el mouse no se mueve pero el contenido bajo
  // el cursor sí: reevaluamos en la última posición para no dejar el cursor pegado
  // en modo drag tras salir del slider deslizándolo.
